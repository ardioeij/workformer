...
def reverse_ssf_label_to_json(expr: str) -> str:
...
def main():
    file_path = "dataset/trimmed_small_workflows_label.txt"
    input_strings = read_input_file(file_path)

    error_count = 0
    for i, input_string in enumerate(input_strings):
        try:
            parsed_json = reverse_ssf_label_to_json(input_string)
...
def test():
    ...
    { In [ String Object Object Object ] Out [ Object ] Seq [
        ...
        { LOOP Seq [ ] Exp [ LNoR ] }
        { LOOP Seq [ { Create a Figcaption node } { Create a new instance of Hyperlink } { Sets the active view model } ] Exp [ LNoR ] }
        { IF Seq [ { ELSE Seq [ { Creates a video node } ] Exp [ LTrue ] } ] }
        { IF Seq [ { ELSE Seq [ { Prompts the user to save a file } { Set AppStatus to Ready } ] Exp [ LTrue ] } ] }
        { 
            IF Seq [ 
                { ELSE Seq [ { Try to get a list of lines from a text file } ] Exp [ LTrue ] } 
                { ELSE Seq [ { Method to try to find an error } { Returns the number of splits to use for precentige splitting } ] Exp [ LTrue LNoR ] } 
                { ELSE Seq [ { Create a DictionaryStreamConfiguration } ] Exp [ ] } 
            ] 
        }
    ] }
    """

    #input_text = "{ In [ Decimal ] Out [ Decimal ] Seq [ { IF Seq [ {  } ] } { 1f = > 1f 1f = > 1f } ] }"
    
    #input_text = "{ In [ Decimal Decimal Decimal ] Out [ Decimal ] Seq [ { Returns a random number between a and b } { IF Seq [ {  } ] } { a - > num t - > t } ] }"

    parsed_output = reverse_ssf_label_to_json(input_text.strip())
    print(parsed_output)
