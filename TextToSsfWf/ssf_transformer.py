...
class PositionalEmbedding(tf.keras.layers.Layer):
  ...
  def call(self, x):
    ...
    pe_slice = tf.slice(self.pos_encoding, [0, 0], [length, -1])
    x = x + pe_slice[tf.newaxis, :, :]
    return x
...
class FeedForward(tf.keras.layers.Layer):
  def __init__(self, d_model, dff, dropout_rate=0.1, activation_type='relu'):
    super().__init__()

    self.seq = tf.keras.Sequential([
      tf.keras.layers.Dense(dff, activation=activation_type),
...
class EncoderLayer(tf.keras.layers.Layer):
  def __init__(self,*, d_model, num_heads, dff, dropout_rate=0.1,
    ...
    self.ffn = FeedForward(d_model, dff, dropout_rate=dropout_rate, activation_type=activation_type)
...
class Transformer(tf.keras.Model):
  def __init__(self, *, encoder_layers, decoder_layers, d_model, num_heads, dff,
               input_vocab_size, target_vocab_size, dropout_rate=0.1, activation_type = 'relu'
               ):
    super().__init__()
    ...
    self.decoder = Decoder(num_layers=decoder_layers, d_model=d_model,
                           num_heads=num_heads, dff=dff,
